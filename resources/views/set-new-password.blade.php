        ...
        <div class="min-vh-100 w-35 max-w-30 flex flex-col">
            ...
            <form class="flex flex-col gap-10" method="POST" action="/account/verify/password/{{$token}}">
                ...
                <div class="mb-2">
                    ...
                    <input type="password" id="password" name="password" class="{{$errors->has('password') ? 'border-error-red' : 'border-borderCl'}} {{!$errors->has('password') && $errors->any() ? 'border-green' : 'border-borderCl'}} shadow-sm bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:shadow-sm-light" placeholder="{{__('messages.new_password')}}">
                    @error('password')
                ...
                <button type="submit" class="text-white bg-green hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 font-bold">{{__('messages.save_changes')}}</button>
            </form>
        </div>
    </div>
</x-layout>